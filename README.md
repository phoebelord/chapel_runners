...
```bash
chapel-runners$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
chapel-runners$ sam local start-api
chapel-runners$ curl http://localhost:3000/
```
...
```bash
chapel-runners$ sam logs -n HelloWorldFunction --stack-name chapel-runners --tail
```
...
```bash
sam delete --stack-name chapel-runners
```
